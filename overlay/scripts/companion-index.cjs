...
trace.emit("index", "start", { corr: key, shortid, copy: "overlay-scripts" });
if (!shortid) {
...
// Collect ALL matches so we can flag the FORK HAZARD (more than one live file
// sharing a shortid). Behavior is unchanged — the first match still wins.
let unitKey = null;
let source = null;
let matches = [];
try {
