// Pixel-art crab scene library for the overlay (idle surfaces).
...
// the splash overlay (.cmp-submitted) to a reusable .crab-stage wrapper so it can
...
export const CW_BASE =
  ".crab-stage .halo{position:absolute;top:50%;left:50%;width:200px;height:200px;transform:translate(-50%,-64%);background:radial-gradient(circle,rgba(217,138,92,.30) 0%,transparent 62%);filter:blur(6px);animation:cwHalo 4.5s ease-in-out infinite;pointer-events:none}" +
  ...
  ".crab-stage .cw{width:150px;height:150px;shape-rendering:crispEdges;overflow:visible;will-change:transform;isolation:isolate;transform:translateZ(0)}" +
  ".crab-stage .cw-shadow{fill:#201b15;opacity:.4}" +
  // sensible idle defaults so a scene only overrides what it animates
  ".crab-stage .cw-eyes-b{transform-origin:7.5px 9px;animation:cwBlink 4.2s steps(1) infinite}" +
  ...
  "@media (prefers-reduced-motion:reduce){.crab-stage .cw *,.crab-stage .halo{animation:none!important}.crab-stage .cw-bit,.crab-stage .cw-load,.crab-stage .cw-str,.crab-stage .cw-pk,.crab-stage .cw-dust,.crab-stage .cw-wave,.crab-stage .cw-star,.crab-stage .cw-q,.crab-stage .cw-sp{opacity:.6!important}}";
...
export const CW_SCENES = [
  { id:"typing", cap:"Your agent’s heads-down in the code",
    ...
    css:
      ...
      ".crab-stage .cw--typing .cw-arm-l{transform-origin:2px 10px;animation:cwTypeL .16s ease-in-out infinite}" +
      ".crab-stage .cw--typing .cw-arm-r{transform-origin:13px 10px;animation:cwTypeR .13s ease-in-out infinite}" +
      ...
      ".crab-stage .cw--typing .cw-eyes{animation:cwRead 1.2s steps(1) infinite}" +
  ...
  { id:"thinking", cap:"Your agent’s turning it over",
    ...
    css:
      ...
      ".crab-stage .cw--thinking .cw-arm-r{transform-origin:14px 10px;animation:cwTap .8s ease-in-out infinite alternate}" +
  ...
  { id:"conducting", cap:"Your agent’s orchestrating the next move",
    ...
    css:
      ...
      ".crab-stage .cw--conducting .cw-arm-l{transform-origin:1px 10px;animation:cwCondL 2s ease-in-out infinite}" +
      ".crab-stage .cw--conducting .cw-arm-r{transform-origin:14px 10px;animation:cwCondR 2s ease-in-out infinite}" +
  ...
  { id:"juggling", cap:"Your agent’s juggling a few threads",
    ...
    css:
      ...
      ".crab-stage .cw--juggling .cw-arm-l{transform-origin:1px 10px;animation:cwJugL .6s ease-in-out infinite alternate}" +
      ".crab-stage .cw--juggling .cw-arm-r{transform-origin:14px 10px;animation:cwJugR .6s ease-in-out infinite alternate}" +
      ...
      ".crab-stage .cw--juggling .cw-eyes{animation:cwDart 1.2s infinite}" +
  ...
  { id:"sweeping", cap:"Your agent’s sweeping up the details",
    ...
    css:
      ...
      ".crab-stage .cw--sweeping .cw-arm-l{transform-origin:1px 10px;transform:translate(6px,1px) rotate(-15deg)}" +
      ".crab-stage .cw--sweeping .cw-arm-r{transform-origin:14px 10px;animation:cwSwArm 1.5s ease-in-out infinite}" +
      "@keyframes cwSwArm{0%,100%{transform:rotate(0deg)}50%{transform:rotate(-20deg)}}" +
      ".crab-stage .cw--sweeping .cw-broom{transform-origin:13.5px 14px;animation:cwBroom 1.5s ease-in-out infinite}" +
  ...
  { id:"beacon", cap:"Your agent’s directing the subagents",
    ...
    css:
      ...
      ".crab-stage .cw--beacon .cw-arm-l{transform-origin:1px 10px;transform:rotate(15deg)}" +
      ".crab-stage .cw--beacon .cw-arm-r{transform-origin:14px 10px;transform:rotate(-15deg)}" +
      ".crab-stage .cw--beacon .cw-wave{transform-origin:7.5px 5px;opacity:0;animation:cwWave 2s ease-out infinite}" +
  ...
  { id:"wizard", cap:"Your agent’s working some magic",
    ...
    css:
      ...
      ".crab-stage .cw--wizard .cw-arm-l{transform-origin:1px 10px;animation:cwWizL 3s ease-in-out infinite}" +
      "@keyframes cwWizL{0%,100%{transform:rotate(20deg)}50%{transform:rotate(120deg)}}" +
      ".crab-stage .cw--wizard .cw-arm-r{transform-origin:13px 10px;animation:cwWizR 3s ease-in-out infinite}" +
      ".crab-stage .cw--wizard .cw-wand{transform-origin:14px 10px;animation:cwWizR 3s ease-in-out infinite}" +
  ...
  { id:"confused", cap:"Your agent’s puzzling it out",
    ...
    css:
      ...
      ".crab-stage .cw--confused .cw-arm-l{transform-origin:1px 10px;transform:translate(0,-2px) rotate(18deg)}" +
      ".crab-stage .cw--confused .cw-eyes{animation:cwLookEye 6s ease-in-out infinite}" +
  ...
  { id:"happy", cap:"Your agent’s pretty pleased with that",
    ...
    css:
      ...
      ".crab-stage .cw--happy .cw-arm-l{transform-origin:2px 10px;animation:cwHapL .15s ease-in-out infinite alternate}" +
      ".crab-stage .cw--happy .cw-arm-r{transform-origin:13px 10px;animation:cwHapR .15s ease-in-out infinite alternate}" +
...
let crabCssInjected = false;
function ensureCrabCss(): void {
  if (crabCssInjected) return;
  const st = document.createElement("style");
  st.id = "crab-css";
  st.textContent = CW_CSS;
  document.head.appendChild(st);
  crabCssInjected = true;
}

/** Render one scene wrapped in its .crab-stage scope (string form). */
export function crabStageHTML(sceneId?: string): string {
  const s = sceneId
    ? (CW_SCENES.find((x) => x.id === sceneId) ?? CW_SCENES[0])
    : CW_SCENES[Math.floor(Math.random() * CW_SCENES.length)];
  return `<div class="crab-stage">${cwSceneSVG(s)}</div>`;
}

/** Mount a fresh crab into `el`. A new random pose each call keeps idle surfaces
 *  feeling alive without any timer (re-entering the surface re-rolls). */
export function mountCrab(el: HTMLElement, sceneId?: string): void {
  ensureCrabCss();
  el.innerHTML = crabStageHTML(sceneId);
}
