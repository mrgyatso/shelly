# Publishing to the Shelly Hub (for agents)
...
Shelly overlay (on their Mac) and the hub's web UI.
...
> To find the data dir: it's whatever `SHELLY_HUB_DATA_DIR` was set to when the
> hub was started, else `~/.shelly/`. Check the hub's startup log — it
...
- **`DATA_DIR`** — the hub's data dir, for Option A. Default `~/.shelly/`.
