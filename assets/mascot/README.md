# Clawd the hermit crab — mascot assets

Clawd, the app's pixel crab, tucked into a hermit-crab shell — because Companion is a
"shell" for Claude. Same sprite style as the animated mascot (`overlay/src/clawd.ts`):
...
- `clawd-hermit.png` — hero render, transparent, 1024² (landscape composition).
...
- **Idle / "waiting for an artifact" splash** → the `hermit` scene in
  `overlay/src/clawd.ts` (a static pixel shell behind the shared rig; left claw waves).
  It joins the random pose rotation on `#hub-clawd` and `#unit-blank-clawd`.

## Provenance

The hero was generated with OpenAI `gpt-image-2` (`images.edit`) using a rasterized
reference of the real Clawd rig to keep the character consistent, then its white
background was flood-filled to transparent. The in-app `hermit` pose is hand-authored
vector pixels (not the raster), matching the other `clawd.ts` scenes.
