...
- **Work here:** worktree `~/companion-identity-registry`, branch `feat/identity-registry` (forked from
  `master` @ `bb7fc80`). Build and commit in THIS worktree only.
- **Do NOT touch `~/claude-code-companion`** (the main checkout) — a concurrent session has uncommitted
...
- **The repo:** a Tauri (Rust + TS/Vite) overlay app under `overlay/`, plus a Claude Code plugin under
  `plugin/`. Remote: `git@github.com:mrgyatso/claude-code-companion.git`. Default branch `master`.
- **Already shipped on `master`** (do not redo; build on top):
  ...
  - `bb7fc80` — **hook cleanup**: the redundant `~/.claude/settings.json` PostToolUse entry was removed
    and `companion open` dropped, so PostToolUse now fires ONCE, via the plugin.
...
### 2.1 The state files (`~/.claude/companion/`)
...
- `companion-session` (SessionStart) — derives identity via `companion-livepath.sh`, writes the live
  stub if absent, and writes the sidecars (`owned-sessions` if `COMPANION_SESSION` env set,
...
- `companion-livepath.sh` — the identity derivation: reuse-existing-by-shortid (frozen identity) vs
  fresh-derive (slug from gitroot/cwd). Prints a TAB line `live_path\tproject\tshortid\tis_repo\tunit_key`.
- `companion-hook` (PostToolUse Write|Edit) — on an artifact write into the artifacts dir, calls
  `companion-index.cjs` to stamp the index. (`overlay/scripts/companion-hook` is a now-dormant older
  copy — settings.json no longer invokes it; the plugin copy is live.)
- `companion-index.cjs` — globs `live/*--<shortid>.json` for the writing session, reads its `unit_key`,
  stamps `artifact-index.json`. **First-match-wins glob = a fork hazard** (logs `matchCount`).
- `companion-session-end` (SessionEnd) — deletes the live file + sidecar entries, but ONLY on genuine
  user-initiated ends (`clear|logout|prompt_input_exit`); keeps everything on forced/crash exits.
- `companion-trace.{cjs,sh}` — the shared NDJSON trace appender (§4). `companion-consider` — Stop hook.
...
- `live.rs` — `read_all_live()` (`#[tauri::command]`, ~258): reads every `live/*.json`, and
  `inject_fields()` (~343) injects `updated_ms`, `companion_session` (from `owned-sessions`), `unit_dir`
...
1. **Authoritative session record** — `~/.claude/companion/sessions/<session_id>.json`, written ONCE at
...
2. **Artifacts reference their owning `session_id`.** The PostToolUse hook records `path → session_id`
   (a thin index, or write the id into the artifact's `companion-meta`). Unit is resolved
...
3. **Append-only event log** = source of truth for "what happened when" (promote the harness shape):
   `~/.claude/companion/events.ndjson`, one event per line:
...
- **Turn on:** `touch ~/.claude/companion/logs/trace.on` (or env `COMPANION_TRACE=1`). **Off:**
  `rm` the flag. Off by default; non-blocking; structured fields only (never raw stdin/env).
- **One NDJSON log:** `~/.claude/companion/logs/trace.ndjson`, joined on `corr` (artifact abs path),
...
The observer lives at `~/companion-artifact-observer` (branch `feat/async-artifact-observer`), plugin
`companion@companion-observer-dev`. It is BOTH:
- **A fork of the identity hooks** — `plugin/hooks/{companion-session,companion-livepath.sh,
  companion-index.cjs,companion-hook,companion-session-end}` are copies of the same identity machinery.
- **A second artifact producer** — `plugin/scripts/artifact-observer/`:
  - `capture.cjs` (a hook) reads the session turn from stdin, detects substantive/visual intent,
    enqueues work; per-session turn state under `~/.claude/companion/observer/sessions/`.
  - `worker.cjs` — a background daemon (queue `~/.claude/companion/observer/queue`, debounce 30s, idle
...
1. **One shared identity implementation both plugins source** — extract the registry + resolution +
   event-append logic into a single lib (e.g. `plugin/hooks/companion-identity.cjs` and/or a small
...
- **Overlay (Rust/TS) build:** `cd ~/companion-identity-registry/overlay && npm run tauri build`
  (~few min; first build slower). Produces `src-tauri/target/release/bundle/macos/Companion Overlay.app`.
- **Install + run:** `pkill -f "Companion Overlay"; rm -rf "/Applications/Companion Overlay.app";
  ditto "<built .app>" "/Applications/Companion Overlay.app"; open "/Applications/Companion Overlay.app"`.
...
- **Phase 1 — dual-write registry (non-breaking).** Add the shared identity lib; `companion-session`
...
## 8. Regression case matrix (run every phase, `COMPANION_TRACE=1`)
...
- **The LaunchAgent strips shell env**, so the overlay daemon won't see `COMPANION_TRACE`. The
...
- **Shared working tree:** committing in this worktree is fine; never edit files in `~/claude-code-companion`.
- **The Board is a sandboxed, opaque-origin iframe host** — artifacts self-report size; the webview
  ...
  to write events).
- **There are ~8 stale `claude-code-companion--*` live files** in `live/` from prior sessions; don't
  assume one session per unit when testing.

---

## 10. Supporting context (read if you need more)
- Agent memories (in `~/.claude/projects/-Users-gyatso-claude-code-companion/memory/`):
  `surfacing-lag-root-cause`, `companion-identity-registry-redesign` (points here),
  `terminal-rehydrate-on-restart`.
- Handoffs at the main repo root: `HANDOFF-2026-06-28-logging-harness.md` (the harness mission),
  `HANDOFF-2026-06-28-hook-cleanup.md` (the double-registration / `companion open` cleanup + Finding B).
- The harness commit `364fc2b` body has the full layer-by-layer description.

---

## 11. First action when you pick this up
1. `cd ~/companion-identity-registry` and confirm `git branch` shows `feat/identity-registry`.
2. Read this plan fully.
3. Do **Phase 0**: build the overlay here, `touch ~/.claude/companion/logs/trace.on`, run the §8 matrix
