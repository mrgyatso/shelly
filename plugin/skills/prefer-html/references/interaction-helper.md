...
<script>
(function () {
  // HANDOVER GUARD — run-second-stand-down.
  //
  // This helper reaches a page two ways now: pasted in by the authoring agent, and
  // injected at write time by `shelly-frame.cjs`. Both can be present at once, and two
  // live copies is not a cosmetic problem — each would mount its own comment box and
  // chat bar, and every ballot click would be handled twice, so the second handler
  // toggles off the state the first just set. The user sees a button that does nothing.
  //
  // Scripts run in document order and the injected frame is always last, so the copy
  // that runs SECOND is the one that stands down: whoever arrived first already owns the
  // page, and its behaviour is the one the artifact was authored against. Three
  // signatures, because copies pasted before this guard existed can't set the flag:
  // the flag itself, `__cmpShowSubmitted` (defined at the end of a full copy), or a
  // gutter icon already in the DOM (an ambient-only copy).
  if (
    window.__shellyHelperMounted ||
    window.__cmpShowSubmitted ||
    document.querySelector(".shelly-ask-btn")
  ) return;
  window.__shellyHelperMounted = true;
  ...
  // SOLE OWNER of the three ballot controls. On `window`, in the CAPTURE phase — ahead
  // of every `document` listener — and it stops the event dead for the controls it owns.
  //
  // The handover guard above only helps when the other copy is a recognisable helper.
  // A hand-rolled ballot handler (the common authoring drift: a few lines that flip
  // data-state and post a submit) sets no flag and draws no gutter icon, so it can't be
  // detected — but it CAN be made inert, which is just as good. Stopping propagation
  // here means it never sees the click, so it can't double-toggle.
  //
  // Deliberately scoped to [data-action] / [data-doall] / [data-shelly-submit]: nothing
  // else on a page legitimately handles those, and every other click the interior owns —
  // blob expand/collapse, wizard Next/Back, the multi-page router, links — is untouched.
  // That is the whole reason this is a capture-phase veto rather than stripping the
  // interior's scripts: stripping would take the artifact's own behaviour with it.
  window.addEventListener("click", function (e) {
    if (!e.target || !e.target.closest) return;
    if (!e.target.closest("[data-action], [data-doall], [data-shelly-submit]")) return;
