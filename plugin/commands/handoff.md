...
the Shelly overlay. This closes the loop from "write a handoff" to "the handoff is running
...
Shelly handoffs dir (durable, no wiki-confirmation needed):

```
~/.shelly/handoffs/<YYYYMMDD-HHMMSS>-<short-slug>.md
```

- `mkdir -p ~/.shelly/handoffs` first.
...
```bash
shelly handoff "<absolute-handoff-path>" --dir "<dir>" [--agent <agent>]
```

The Shelly overlay spawns a fresh session in `<dir>`, then auto-sends
...
If the `shelly` command is not found, the overlay/CLI isn't installed: tell the user to
install with `brew install --cask mrgyatso/tap/shelly`, then the file you wrote
is still a valid handoff they can open manually.

## 3. Report

Print the absolute path of the handoff you wrote and the exact `shelly handoff …` command
